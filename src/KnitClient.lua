...
--[=[
	@interface ComponentExtensionsInterface
	.ShouldExtend ((component: Component) -> boolean)?
	.ShouldConstruct ((component: Component) -> boolean)?
	.Constructing ((component: Component) -> boolean)?
	.Constructed ((component: Component) -> boolean)?
	.Starting ((component: Component) -> boolean)?
	.Started ((component: Component) -> boolean)?
	.Stopping ((component: Component) -> boolean)?
	.Stopped ((component: Component) -> boolean)?
	@within KnitClient
]=]
type ComponentExtensionsInterface = {
	ShouldExtend: ((component: Component) -> boolean)?,
	ShouldConstruct: ((component: Component) -> boolean)?,
	Constructing: ((component: Component) -> boolean)?,
	Constructed: ((component: Component) -> boolean)?,
...
--[=[
	...
	.Tag string
	.Ancestors {Instance}
	.Extensions : ComponentExtensionsInterface
	.[any] any
	@within KnitClient
]=]
type ComponentDef = {
...
local Component = require(KnitClient.Util.Component)
...
--[=[
	Requires all the modules that are children of the given parent. This is an easy
	way to quickly load all components that might be in a folder.

	@error No tag was specified in component -- Every Component object requires a CollectionService tag string at index Tag. If one is not specified the AddComponents method will error.
	```lua
	Knit.AddComponents(somewhere.Components)
	```
]=]
